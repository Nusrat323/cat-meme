...
export default function Call({ keyText = "Talking with my man ❤️", skipRingtone = false, isNight = false }) {
  ...
  useEffect(() => {
    ...
    if (ringRef.current && callRef.current) {
  ...
  return (
    <div
      className={`relative w-full max-w-md sm:max-w-lg md:max-w-2xl aspect-[1/1] flex items-end justify-center
                  rounded-3xl shadow-xl overflow-hidden p-4 mx-auto ${bgClass}`}
    >
      {/* Night stars */}
      ...
      {/* Cloud bubble */}
      <motion.div
        className="absolute top-8 right-6 flex flex-col items-center justify-center"
        animate={{ y: [0, -8, 0], x: [0, 5, 0], scale: [1, 1.03, 1] }}
        transition={{ repeat: Infinity, duration: 2 }}
      >
        <div
          className="relative bg-gray-100 p-3 rounded-2xl shadow-lg flex items-center justify-center border border-purple-100"
          style={{
            width: "130px",
            height: "85px",
